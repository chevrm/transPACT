...
my %p = ();
open my $bfh, '<', './KS_precomputed_1405_hmmalign_trimmed_renamed.annotationtable.tsv';
while(<$bfh>){
    chomp;
    next if($_ =~ m/^\s/);
...
my %isce = ();
open my $cfh, '<', '../dendrogram20200227/leaftbl.tsv' or die $!;
while(<$cfh>){
    chomp;
    my ($p, $ce) = split(/\t/, $_);
    if($ce eq 'Yes'){
...
open my $efh, '>', 'itol_edge.txt' or die $!;
