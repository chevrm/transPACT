     ...
     while True:
         if (time.time() - processes_starttime) > 1200:
             if sys.platform == ('linux2') or sys.platform == ('darwin'):
                 ...
                 logging.warning("Now in " + os.getcwd() + " TreeGraph -convert on tree" + str(inputnr) + " ran out out of time")
                 break
     ...
     if "exception" not in output and "Exception" not in output:
         ...
         while True:
             if (time.time() - processes_starttime) > 1200:
                 if sys.platform == ('linux2') or sys.platform == ('darwin'):
                     os.kill(p.pid,signal.SIGKILL)
                     logging.warning("Now in " + os.getcwd() + " TreeGraph -image on tree " + str(inputnr) + " ran out out of time")
                     break
                 elif sys.platform == ('win32'):
                     subprocess.Popen("taskkill /F /T /PID %i"%p.pid , shell=True, stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
                     logging.warning("Now in " + os.getcwd() + " TreeGraph -image on tree " + str(inputnr) + " ran out out of time")
